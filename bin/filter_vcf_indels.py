...
# count how many alleles have a given base count
# e.g. CG would be {2:1, ...} then TA would be {2:2,... } then T would be {1:1, 2:2,...)
# {alleleBaseCount: alleleCount, ...}
distribDict = {}

inFile = sys.stdin
logFile = os.getenv('MGP_VCF_LOG_FILE')
distribLogFile = os.getenv('MGP_VCF_DISTRIB_LOG_FILE')
...
def processFile():
    ...
    while line:
        ...
        if str.find(line, '#') == 0:
            ...
            ref = tokens[3]
            refLen = len(ref)
            if refLen not in distribDict:
                distribDict[refLen] = 1
            else:
                distribDict[refLen] += 1
            alt = tokens[4]
            qual = tokens[5]
            ...
            if str.find(alt, ',') != -1: # multiple alleles
                multiCt += 1
                mAlleles = str.split(alt, ',')
                for a in mAlleles:
                    altLen= len(a)
                    if altLen not in distribDict:
                        distribDict[altLen] = 1
                    else:
                        distribDict[altLen] += 1
...
def writeLog():
    ...
    fpDistribLog.write('baseLength%salleleCount%s' % (TAB, CRT))

    sortedBaseLengthList = sorted(distribDict.keys())
    for key in sortedBaseLengthList:
        fpDistribLog.write('%s%s%s%s' % (key, TAB, distribDict[key], CRT))
    return 0
...
def openFiles():
    ...
    try:
        fpDistribLog = open(distribLogFile, 'w')
    except:
        return 1
